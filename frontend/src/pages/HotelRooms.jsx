...
const HotelRooms = () => {
  ...
  return (
    <div className="mt-15 md:mt-30 w-full xl:px-0 px-6 xl:w-5/6 mx-auto ">
      <Title
        title="Hotel Rooms"
      ...
      <div className="flex lg:flex-row flex-col-reverse">
        ...
        <div className="flex flex-col gap-12 mt-10">
            {  
                filteredRooms.slice(0,4).map((data)=>{
                    return(
                        <div key={data._id} className="flex sm:flex-row flex-col gap-6 ">
                            <img onClick={()=> navigate(`/rooms/${data._id}`)} src={data.image[0]} className="rounded-xl w-full sm:w-3/6 lg:w-2/6"  alt="roomimage" />

                            <div className="flex flex-col gap-5">
                                <p className="text-gray-600 lg:text-lg text-md">{data.hotel.address}</p>
                                <h1 onClick={()=> navigate(`/rooms/${data._id}`)} className="text-2xl lg:text-3xl text-gray-800 font-playfair">{data?.hotel?.name}</h1>

                                {/* Rating */}
                                <div className="flex gap-13">
                                    <Rating/>
                                    <span className="lg:text-lg text-md">200+ reviews</span>
                                </div>

                                {/* LOcation */}
                                <div className="flex gap-4">
                                    <img src={assets.locationIcon} alt="locationimage"/>
                                    <span className="text-gray-600 lg:text-lg text-md">{data.hotel.address}</span>
                                </div>
                                        ...
                                        data.amenities.map((aminity, index)=>{
                                            return(
                                                <div key={index} className="flex  bg-gray-200 px-2 py-1 rounded-sm gap-2">
                                                    <img className="w-3 lg:w-4" src={facilityIcons[aminity]} alt="aminityimage"/>
                                                    <p className="text-sm lg:text-lg">{aminity}</p>
                                                </div>
                                            )
                                        })
                                    }
                                ...
                                {/* charge per day */}
                                <p className="text-lg lg:text-2xl text-gray-700">{currency}{data.pricePerNight}/night</p>
                            </div>
                        </div>
                    )
                })
        ...
        {/* filters */}
        <div className="w-full sm:w-3/6 lg:w-2/6 border border-gray-400 flex flex-col gap-3 h-fit pb-6">
            <div className="flex justify-between border-b gap-15 border-gray-400">
                ...
                <div className="flex items-center pr-3 text-md text-gray-400">
                    ...
                    <span className=" lg:block hidden lg:py-2 lg:px-2 px-0 py-0">CLEAR</span>
                </div>
            </div>
